...
char	*get_next_line(int fd)
{
	...
	if (!buffer)
		return (NULL);
	line = ft_get_line();// ft_get_line renvoie la str lu par read mais couper au \n
	buffer = ft_new_bufffer_start();// cette fonction a pour but de recuperer le reste de la str pris en compte dans ft_get_line
	return (line);
}